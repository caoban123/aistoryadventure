...
\usepackage[T5]{fontenc} % Supports Vietnamese font encodings
...
\hypersetup{
    ...
    filecolor=magenta,      
    urlcolor=cyan,
    pdftitle={AI Story Adventure - Project Report},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{AI Story Adventure - Báo cáo dự án}
\fancyhead[R]{\thepage}
\fancyfoot[C]{Hệ thống Kể chuyện Tương tác AI}
...
\begin{titlepage}
    \centering
    \vspace*{2cm}
    {\scshape\LARGE Trường Đại học Kỹ thuật \par}
    \vspace{1.5cm}
    {\scshape\Large BÁO CÁO MÔN HỌC / ĐỒ ÁN DỰ ÁN\par}
    \vspace{1.5cm}
    {\huge\bfseries AI STORY ADVENTURE\par}
    \vspace{0.5cm}
    {\large\itshape Hệ thống kể chuyện tương tác thời gian thực với bộ nhớ ngữ nghĩa dài hạn và kiến trúc tự động triển khai\par}
    \vspace{2cm}
...
% ==========================================
% SECTION 1: Motivation và mục tiêu hệ thống
% ==========================================
...
\begin{itemize}
    ...
    \item \textbf{Kết quả:} Hệ thống không bị sập hoàn toàn nhờ cơ chế xử lý lỗi ngoại lệ (`try-catch`) phân lớp. Khi cơ sở dữ liệu vector gặp sự cố, hệ thống tự động hạ cấp xuống chế độ bộ nhớ đệm hội thoại ngắn hạn (fallback to memory buffer) để người chơi tiếp tục phiên mà không bị mất trải nghiệm.
\end{itemize}
...
% ==========================================
% SECTION 3: Metric evaluation và kết quả
% ==========================================
...
\begin{itemize}
    ...
    \item \textbf{Đánh giá trải nghiệm đọc (Độ hài lòng đạt 4.6/5.0):} Người dùng đánh giá cao việc cải tiến giao diện căn giữa nội dung truyện theo chiều dọc (`max-width: 72ch` cùng lề `auto`), giúp giảm mỏi mắt khi đọc trên màn hình lớn. Khoảng cách đệm dưới được mở rộng giúp ngăn việc các đoạn chữ cuối cùng bị che khuất bởi thanh công cụ nhập liệu.
\end{itemize}

\newpage

% ==========================================
% SECTION 4: Cách deploy/demo
% ==========================================
...
\begin{enumerate}
    \item \textbf{Cấu hình biến môi trường (`.env`):} Khai báo khóa API OpenAI, Gemini, cấu hình Firebase Admin SDK và thông tin kết nối Qdrant.
    \item \textbf{Đóng gói Docker:} Khởi chạy container backend với cổng `8000` được ánh xạ ra ngoài.
...
\begin{itemize}
    ...
    \item \textbf{Cache Busting (Chống lưu đệm cũ):} Để đảm bảo người chơi luôn nhận được những bản cập nhật CSS/JS mới nhất (như cải tiến thanh điều hướng topbar, cơ chế accordion cho thông báo), các liên kết nhúng trong `index.html` được thêm số hiệu phiên bản truy vấn (ví dụ: `style.css?v=player-app-38` và `app.js?v=player-app-39`).
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Quản lý và định tuyến:} Sử dụng Coolify để tự động cập nhật mã nguồn mỗi khi có thay đổi được đẩy lên nhánh `main` của kho lưu trữ GitHub. Cloudflare Tunnel được sử dụng để thiết lập đường truyền mã hóa bảo mật từ VPS tới người dùng cuối mà không cần mở cổng public trực tiếp trên tường lửa máy chủ.
\end{itemize}

\newpage

% ==========================================
% SECTION 5: Reflection checklist
% ==========================================
...
\begin{enumerate}
    ...
    \item \textbf{Quản lý dòng sự kiện (Event Propagation) trong JavaScript:} Khi thiết kế các nút hành động nằm lồng bên trong thẻ bọc có sự kiện nhấp chuột (như nút Dismiss nằm trong thẻ Thông báo accordion), phải luôn sử dụng `event.stopPropagation()` để ngăn chặn hiện tượng kích hoạt ngoài ý muốn sự kiện của thẻ cha.
    \item \textbf{Thứ tự đăng ký tuyến đường (Route order) trong FastAPI:} Các tuyến đường tĩnh hoặc tuyến đường cụ thể (ví dụ: `/game/me`) phải luôn được khai báo trước các tuyến đường động có tham số biến (ví dụ: `/game/{session_id}`), nếu không FastAPI sẽ nhận diện sai và dẫn đến lỗi không tìm thấy trang (HTTP 404).
\end{enumerate}

\newpage

% ==========================================
% SECTION 6: Action item cho lần cải tiến tiếp theo
% ==========================================
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Rà soát toàn bộ tệp tin trong thư mục `app/` để xóa bỏ thư viện ChromaDB khỏi `requirements.txt`.
